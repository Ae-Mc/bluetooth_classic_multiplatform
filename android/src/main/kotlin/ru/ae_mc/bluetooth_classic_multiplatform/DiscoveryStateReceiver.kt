...
class DiscoveryStateReceiver : EventChannel.StreamHandler {
    companion object {
        const val CHANNEL_NAME: String = "${Helper.NAMESPACE}/discoveryState"
    }
    ...
    val discoveryStateReceiver: BroadcastReceiver =
            object : BroadcastReceiver() {
                override fun onReceive(context: Context, intent: Intent) {
                    val action = intent.action ?: return
                    adapterStateEventSink.let {
                        it?.success(action == BluetoothAdapter.ACTION_DISCOVERY_STARTED)
                    }
                }
            }
}
