...
import ru.ae_mc.bluetooth_classic_multiplatform.BluetoothClassicMultiplatformPlugin.Companion.TAG
...
abstract class BluetoothConnection(private val bluetoothAdapter: BluetoothAdapter) {
     ...
     *
     * @return `true` if a connection is active and not in the process of closing, `false`
     * otherwise.
     */
    ...
    fun connect(address: String?, uuid: UUID?) {
        ...
        val device =
                bluetoothAdapter.getRemoteDevice(address) ?: throw IOException("device not found")
        val socket =
                device.createRfcommSocketToServiceRecord(uuid ?: DEFAULT_UUID)
     ...
     *
     * This function checks if a connection is currently active. If it is, it cancels the ongoing
     * connection thread and sets it to null, effectively closing the connection. If no connection
     * is active, this function does nothing.
     */
    fun disconnect() {
        if (isConnected()) {
     ...
     *
     * This function attempts to send the provided byte array [data] to the remote device through
     * the active Bluetooth connection.
     *
     * @param data The [ByteArray] containing the data to be written.
     * @throws IOException if the device is not connected, or if an error occurs during the write
     * operation.
     */
    @Throws(IOException::class)
    ...
    /**
     * Callback for reading data. This function is called when data is received from the connected
     * Bluetooth device. This method should handle the incoming data.
     *
    ...
    private inner class ConnectionThread(private val socket: BluetoothSocket) : Thread() {
        ...
        /**
         * A flag indicating whether a request to close the connection has been made. This is used
         * to signal the connection thread to terminate its operations gracefully. It is marked as
         * `@Volatile` to ensure that changes made by one thread are immediately visible to other
         * threads.
         */
        @Volatile var requestedClosing = false

        /**
         * This method is the main execution loop for the connection thread. It continuously reads
         * data from the input stream and calls the `onRead` callback with the received data. If an
         * error occurs during reading or the connection is closed, the loop terminates. Finally, it
         * calls the `onDisconnected` callback and ensures resources are closed.
         */
        override fun run() {
            try {
         ...
         *
         * This function will attempt to write the provided byte array to the connected Bluetooth
         * socket's output stream. If the connection is closing, this will log a warning and do
         * nothing.
         *
         ...
         *
         * This method attempts to flush the output stream and then close the socket. Any exceptions
         * encountered during these operations are caught and logged, but they do not prevent the
